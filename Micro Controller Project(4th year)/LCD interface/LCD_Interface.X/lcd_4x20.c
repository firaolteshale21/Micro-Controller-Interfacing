...
void main() {
    ...
    char temperatureStatus[16]; // Buffer to hold temperature status
    ...
    while (1) {
        ...
        sprintf(tempStr, "%.1f C   ", temperature); // Convert temperature to string with one decimal place
        Lcd_Out(2, 1, tempStr);
        Lcd_Out(4, 8, temperatureStatus); // Display temperature status on the third row
...
void Delay_ms(unsigned int ms) {
    ...
    for (i = 0; i < ms; i++) {
        for (j = 0; j < 1000; j++) {
            __delay_us(2); // Delay 1 microsecond
        }
    }
}
